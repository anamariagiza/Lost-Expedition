...
public class Map {
    ...
    private int width, height; // Latimea si inaltimea hartii in dale
    private List<int[][]> tilesGidsLayers; /*!< Lista de matrici de GID-uri, una pentru fiecare strat al hartii.*/

    // Constructorul acum primeste RefLinks
    public Map(RefLinks refLink) {
        this.refLink = refLink;
    }

    // Metoda pentru a incarca o harta specifica dintr-un fisier
     ...
     */
    public void Update() {
        // Aici poti actualiza dale animate, daca ai
    }
     ...
     */
    // @Override // Am eliminat @Override deoarece clasa Map nu extinde State sau o alta clasa cu metoda Draw
    public void Draw(Graphics g) {
        if (tilesGidsLayers == null || tilesGidsLayers.isEmpty()) {
            return; // Nu desena nimic daca nu sunt straturi incarcate
        }

        GameCamera camera = refLink.GetGameCamera();
        float zoom = camera.getZoomLevel();

        // Calculeaza ce portiune a hartii este vizibila pentru a optimiza desenarea
        // Tine cont de zoom aici
        ...
        // NOU: Iteram prin FIECARE strat din lista tilesGidsLayers pentru a le desena pe toate
        for (int[][] currentLayerGids : tilesGidsLayers) {
            for (int y = yStart; y < yEnd; y++) {
                for (int x = xStart; x < xEnd; x++) {
                    int gid = currentLayerGids[x][y];

                    // Daca GID-ul este 0, inseamna un tile gol/transparent in Tiled. Nu il desena.
                    // Aceasta este o optimizare.
                    if (gid == 0) {
                        continue;
                    }
                    ...
                    if (tile != null) {
                        tile.Draw(g, drawX, drawY, scaledTileWidth, scaledTileHeight);
                    } else {
                        // Debugging pentru dale lipsa/invalide
                        g.setColor(Color.RED);
                        g.fillRect(drawX, drawY, scaledTileWidth, scaledTileHeight);
                        System.err.println("DEBUG: Dala cu GID " + gid + " nu a putut fi incarcata sau este invalida.");
                    }
                }
            }
        }
    ...
    private void loadMap(String path) {
        ...
        try {
            ...
            // Iteram prin FIECARE nod <layer> din fisierul TMX
            for (int layerIdx = 0; layerIdx < layerNodes.getLength(); layerIdx++) {
                ...
                tilesGidsLayers.add(currentLayerGids); // Adauga stratul parsate la lista
            }
            is.close();
